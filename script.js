<<<<<<< HEAD
  // API Configuration
      ...
      function initApp() {
        ...
        registerTrigger &&
          registerTrigger.addEventListener("click", showRegistration);
      ...
      function generateOtp() {
        ...
        showNotification(
          "OTP Sent",
          `OTP sent to ${userData.email}: ${userData.otp} (simulated for demo)`
        );
      ...
      function verifyOtp() {
        ...
        if (enteredOtp === userData.otp) {
          ...
          showNotification(
            ...
            `You're all set up with Smart Weather Forecast.`
          );
        } else {
      ...
      async function fetchWeather(location) {
        ...
        try {
          ...
          showNotification(
            "Success",
            `Weather data for ${data.location.name} loaded`
          );
          ...
          showNotification(
            "Error",
            "Failed to fetch weather data. Using backup if available."
          );
          loadBackupData();
        }
      ...
      function updateHourlyForecast(data) {
        ...
        for (let i = 0; i < 12; i++) {
          ...
          hourItem.innerHTML = `
                    ...
                    <div class="day-weather"><i class="fas ${getWeatherIcon(
      ...
      function updateWeeklyForecast(data) {
        ...
        forecastDays.forEach((dayData, index) => {
          ...
          dayCard.innerHTML = `
                    ...
                    )}° / ${Math.round(dayData.day.maxtemp_c)}°</div>
                    <div class="day-stats">
          ...
          if (
            dayData.day.daily_will_it_rain ||
            dayData.day.daily_chance_of_rain > 50
          ) {
            dayCard.innerHTML +=
              '<div class="alert-icon"><i class="fas fa-cloud-rain"></i></div>';
          } else if (dayData.day.maxtemp_c > 30) {
            dayCard.innerHTML +=
              '<div class="alert-icon" style="background: var(--warning);"><i class="fas fa-temperature-high"></i></div>';
          } else if (dayData.day.mintemp_c < 0) {
            dayCard.innerHTML +=
              '<div class="alert-icon" style="background: var(--primary);"><i class="fas fa-snowflake"></i></div>';
          }
      ...
      function updateWeatherAdvisory(data) {
        ...
        if (uvIndex <= 2) uvText = "Low UV index - minimal protection needed";
          ...
          uvText = "Moderate UV index - protection recommended";
        else if (uvIndex <= 7) uvText = "High UV index - protection essential";
        else if (uvIndex <= 10)
          uvText = "Very high UV index - extra protection needed";
        else uvText = "Extreme UV index - avoid sun exposure";

        healthTipsHTML += `<li><i class="fas ${
          uvIndex <= 2
            ? "fa-check-circle"
            : uvIndex <= 7
        ...
        activityTipsHTML += `<li><i class="fas ${
          temp > 30 || temp <= 5
            ? "fa-times-circle"
            : temp > 25
            ? "fa-exclamation-circle"
        ...
        travelTipsHTML += `<li><i class="fas ${
          isRaining && isRushHour
            ? "fa-times-circle"
            : isRaining || isRushHour
            ? "fa-exclamation-circle"
            : "fa-check-circle"
      ...
      function updateWeatherAnimation(condition) {
        ...
        weatherAnimation
          .querySelectorAll(".raindrop, .snowflake, .wind-particle")
      ...
      function addCloud(topPercent, leftPercent) {
        ...
        cloud.style.animation = `cloudFloat ${
          20 + Math.random() * 20
        }s infinite linear`;
      ...
      function toggleSOS() {
        ...
        if (isActive) {
          sosToggle.innerHTML =
            '<i class="fas fa-shield-alt"></i> TRAVEL SAFETY MODE: ON';
          sosToggle.style.background = "var(--success)";
          showNotification(
            "Safety Mode Activated",
            "Travel safety features enabled"
          );
        } else {
          sosToggle.innerHTML =
            '<i class="fas fa-shield-alt"></i> TRAVEL SAFETY MODE: OFF';
      ...
      function shareLocation() {
        showNotification(
          "Location Shared",
          "Your location has been shared with emergency contacts via WhatsApp"
        );
      }
...
>>>>>>> fcce6cf450e8191608263b1d397db3ce4c4b55c7
